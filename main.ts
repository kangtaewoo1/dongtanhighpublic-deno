...
<meta name="description" content="2026년 동탄에서 지금 가장 뜨는 야간 핫플. 동탄 하이퍼블릭 — 프리미엄 사운드, 전용 무대, 동탄 전 지역 무료 픽업. 늦은 밤 동탄에서 즐기는 가장 트렌디한 선택.">
<link rel="canonical" href="https://dongtanhighpublic-deno.kangtaewoo1.deno.net/">
<meta name="robots" content="index,follow,max-image-preview:large">
<meta property="og:type" content="website">
<meta property="og:title" content="동탄 하이퍼블릭 | 2026 동탄 야간 핫플">
<meta name="geo.region" content="KR-41">
<meta name="geo.placename" content="경기도 화성시 동탄">
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
:root{
  --bg:#060606;--surface:#0e0e0e;--surface2:#141414;
  --gold:#C9A84C;--gold2:#E8C96A;--gold-dim:#6b5a28;
  --text:#F0EAD6;--muted:#777;--border:#222;
}
...
.tel-btn{background:transparent;border:1px solid var(--gold);color:var(--gold);padding:9px 22px;border-radius:3px;font-size:.85rem;font-weight:700;letter-spacing:1px;transition:.2s}
.tel-btn:hover{background:var(--gold);color:#000}
...
.hero-orb2{position:absolute;width:400px;height:400px;border-radius:50%;background:radial-gradient(circle,rgba(201,168,76,.04) 0%,transparent 60%);bottom:-100px;left:10%;animation:float 6s ease-in-out infinite reverse}
@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-20px)}}
.hero-content{position:relative;z-index:2;width:100%}
.hero-eyebrow{display:flex;align-items:center;gap:12px;margin-bottom:24px}
.eyebrow-badge{background:rgba(201,168,76,.15);border:1px solid rgba(201,168,76,.3);color:var(--gold);font-size:.72rem;letter-spacing:3px;padding:5px 14px;text-transform:uppercase}
.eyebrow-line{flex:1;max-width:60px;height:1px;background:linear-gradient(to right,var(--gold),transparent)}
...
.btn-primary{display:inline-flex;align-items:center;gap:8px;background:var(--gold);color:#000;padding:14px 32px;font-size:.95rem;font-weight:700;letter-spacing:1px;transition:.2s}
.btn-primary:hover{background:var(--gold2)}
.btn-ghost{display:inline-flex;align-items:center;gap:8px;border:1px solid rgba(201,168,76,.4);color:var(--gold);padding:14px 28px;font-size:.95rem;letter-spacing:1px;transition:.2s}
...
.hero-stat-label{font-size:.75rem;color:var(--muted);margin-top:4px;letter-spacing:.5px}
...
.sec-sub{color:var(--muted);font-size:.95rem;margin-bottom:56px;line-height:1.8}
.feat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--border)}
.feat-card{background:var(--bg);padding:48px 36px;position:relative;overflow:hidden}
.feat-card::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(201,168,76,.04),transparent);opacity:0;transition:.4s}
.feat-card:hover::before{opacity:1}
.feat-glyph{font-size:2.4rem;margin-bottom:20px}
.feat-title{font-size:1.05rem;font-weight:700;margin-bottom:10px;letter-spacing:.3px}
...
.faq-q{padding:22px 28px;font-weight:600;font-size:.95rem;cursor:pointer;display:flex;justify-content:space-between;align-items:center;transition:.2s}
...
.link-card{background:var(--surface);padding:28px 24px;display:flex;align-items:center;gap:14px;transition:.3s;border-top:1px solid var(--border)}
.link-card:hover{border-top-color:var(--gold);background:var(--surface2)}
...
.cta-tel{display:inline-flex;align-items:center;gap:14px;background:var(--gold);color:#000;padding:18px 52px;font-size:1.15rem;font-weight:900;letter-spacing:2px;margin-top:36px;transition:.2s}
.cta-tel:hover{background:var(--gold2);transform:translateY(-2px)}
.cta-note{font-size:.8rem;color:var(--muted);margin-top:16px;letter-spacing:.5px}
...
.floating-tel{position:fixed;bottom:28px;right:28px;z-index:300;background:linear-gradient(135deg,var(--gold),var(--gold2));color:#000;width:58px;height:58px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.3rem;box-shadow:0 4px 28px rgba(201,168,76,.45);transition:.2s}
.floating-tel:hover{transform:scale(1.08)}
@media(max-width:900px){
  .header-inner{padding:14px 20px}
  nav{display:none}
  .hero h1{font-size:2.2rem}
  .feat-grid{grid-template-columns:1fr;background:none;gap:2px}
...
<header id="header">
  <div class="header-inner">
    <div class="logo">
      <div class="logo-mark">동</div>
      <div>
        <div class="logo-text">동탄하이퍼블릭</div>
        <span class="logo-sub">2026 DONGTAN HOTSPOT</span>
      </div>
    </div>
    <nav>
...
<section class="hero">
  ...
  <div class="hero-content wrap">
    ...
    <h1>
      <span class="line1">동탄 밤의 중심,</span>
      <span class="line2">하이퍼블릭</span>
    </h1>
...
<section id="features">
  <div class="wrap">
    ...
    <div class="feat-grid">
      ...
      <div class="feat-card"><div class="feat-glyph">🚗</div><div class="feat-title">동탄 전 지역 무료 픽업</div><div class="feat-desc">동탄역부터 동탄2, 반송동, 병점, 진안동까지. 동탄 어디서든 5~15분 내 무료 픽업. 24시간 운영.</div><span class="feat-tag">FREE PICKUP</span></div>
      <div class="feat-card"><div class="feat-glyph">🏢</div><div class="feat-title">법인·단체 회식 최적화</div><div class="feat-desc">법인카드 결제 가능. 5인 이상 팀 회식, 기업 접대에 특화. 단체 모임을 위한 넓은 공간과 프리미엄 서비스.</div><span class="feat-tag">CORPORATE READY</span></div>
      ...
      <div class="feat-card"><div class="feat-glyph">📍</div><div class="feat-title">동탄 생활권 전체 커버</div><div class="feat-desc">동탄역·동탄2·반송·병점·능동·산척·영천동 전 생활권 서비스. 동탄에 계신다면 어디서든 연결됩니다.</div><span class="feat-tag">FULL COVERAGE</span></div>
    </div>
  </div>
</section>
<section id="vibe" class="vibe-section">
  <div class="wrap">
    ...
    <div class="divider"></div>
    <p class="sec-sub">단순한 노래방이 아닙니다. 프리미엄 공간에서의 다른 경험입니다.</p>
    <div class="vibe-list">
...
<footer>
  <p>동탄 하이퍼블릭 | 2026 동탄 야간 핫플 | 경기도 화성시 동탄 | 24시간 운영</p>
</footer>
